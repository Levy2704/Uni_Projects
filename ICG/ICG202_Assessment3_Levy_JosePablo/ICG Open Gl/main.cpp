...
// CameraFPV
CameraFPV camera(glm::vec3(0.0f, 0.0f, 3.0f)); // make camera at xyz position
float lastX = 1280 / 2, lastY = 720 / 2;
bool firstMouse = true; // help make sure firts mouse read doesn't jump the camera pos
...
void main()
{
	...
	//build a window
	GLFWwindow* window 
		= glfwCreateWindow(1280, 720, " Assessment 3 ICG", NULL, NULL);
	...
	// Hide cursos and lock mouse to window view
...
void mouse_Callback(GLFWwindow* window, double xpos, double ypos)
{
	// if firt reading the mouse, set lastx and y to current mouse positon so 
	// camera doesn't get jerked about
	if (firstMouse)
	{
		lastX = xpos;
		lastY = ypos;
	...
	float yOffset = lastY - ypos; // neeed to reverse to get the correct movement
	//update lastX and Y
	lastX = xpos;
	lastY = ypos;
...
void scroll_Callback(GLFWwindow* window, double xOffset, double yOffset)
{
	camera.ProcessMouseScroll(yOffset); // changes FOV of camera
}
